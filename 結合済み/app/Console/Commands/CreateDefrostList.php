            ...
            foreach($people_by_wakuchin as $rec){
                ...
                foreach($syringes_nums as $syringe){
                    ...
                    if($number_of_people <= ($syringe->syringe_total - $syringe->reserve_inventory)){
                        ...
                        $setRemainder = new Remainder;
                        $setRemainder->wakuchin_ID = $rec->wakuchin_ID;
                        $setRemainder->quantity = $remainder;
                        $setRemainder->hospital_ID = $target->hospital_ID;
                        $setRemainder->remainder_date = date('Y/m/d');
                        $setRemainder->save();
                        ...
                        ->where('wakuchin_stocks.hospital_ID',$target->hospital_ID) 
