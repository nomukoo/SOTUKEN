...
<div class="container-fluid">
  <div class="row">
    ...
    @if(count($hoapitals)!=0)
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h1">登録済病院情報一覧画面</h1>
  ...
  @foreach($hoapitals as $list)
    <tr>
  <td scope="row" class="table-white border-dark" >{{$list->hospital_ID}}</td>
  <td scope="row" class="table-white border-dark" >{{$list->hospital_name}}</td>
  <td scope="row" class="table-white border-dark" >{{$list->hospital_address}}</td>
  <td scope="row" class="table-white border-dark" >{{$list->hospital_tell}}</td>
  <td scope="row" class="table-white border-dark" >{{$list->hospital_ido}}</td>
  <td scope="row" class="table-white border-dark" >{{$list->hospital_keido}}</td>
  </tr>  
...
<div class="col-2">
  <button type="button" class="btn btn-danger rounded-pill " style="width:90px" onclick="history.back(-1)"><h4>戻る</h4></button>
    ...
    @else
      <h2 style="text-align:center">登録された病院はありません</h2>
    @endif
  </body>
</html>